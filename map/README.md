...
2026-09-16 night (Apple's own globe rasters + mesh shading below z 4.6): globe **6.60 %** (t20 25.6 %) — up from 3.74 % because the rasters
...
| `ground-globe`, `ground-globe-ea` (raster images, below PAL) | **Apple's own globe ground** (RENDER-PIPELINE 2.4b, data session `pipeline/basemap/spr_globe.py` ← the SPR tiles of the local geod cache read with GeoServices' `GEOVectorTile`): land-cover class raster (chapter 155) + climate codes (154) → sheet colours × light; world 4096² (z2 tiles, z3 on top) and East Asia lon 90–180 / lat 0–66.51 at z3 resolution; alpha 0 where the cache had no tile (southern hemisphere, Americas) and over Water — the sampled `climate-globe.png` / land fill show through there | `ui/basemap/ground-globe.json` (bounds, class list, HSV tables). Colours are the sheet's (the iOS look); the Mac App's pastel globe = the `DvMt` material colours, not decoded yet (§6) |
