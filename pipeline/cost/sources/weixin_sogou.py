...
CITY_HINT={'urumqi':{'in':['乌鲁木齐','乌市','天山区','沙依巴克区','沙区','新市区','水磨沟区','头屯河区','米东区','高新区','经开区','开发区','红山','南湖','火车站','铁路局','红光山','八道湾','维泰路','友好','小西门','大巴扎','幸福路','北京路','长江路','河滩'],
                     'suburb':['达坂城','乌鲁木齐县'],
...
HEAD_LINE=re.compile(r'^\s*(?:🔥|『地址』|[❶-❿①-⑩]|\d{1,2}\s*[，,.、]\s*|岗位[一二三四五六七八九十\d]+\s*[:：]?|【(?!转正|待遇|福利|要求|工作)|\[庆祝\]|招\s?聘(?!名额|职位|要求|人数|岗位|条件|对象|范围|流程|时间|地址|说明|需求|信息|简章|电话|方式|专业)|急招|诚聘|招(?!聘(?:名额|人数|要求|职位|条件|对象|岗位))[^\n]{0,12}(?:名|人)\b|[^\n]{0,30}(?:厂|公司|店|院|仓|驿站|物业|超市|酒店|餐厅|基地|学校|医院)[^\n]{0,8}招)')
SEP_LINE=re.compile(r'^\s*[-—_=·•～~]{4,}\s*$|长按二维码查看详情|扫二维码报名|应聘时请备注|^\s*DR\d{6,}\s*$')
ACCOUNT_BLOCK=['川渝掌上厨师平台','厨师互帮圈','掌上厨夜推号','听风笑谈','新疆人才途']   # 全国厨师汇总号（几乎全外地）+ 区间薪资正规招聘汇总号（0 命中）：来源不对，不是口径紧
