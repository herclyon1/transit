...
KEYS={
 ...
 'apple1kg':  ('りんご','円/kg','piece',r'ml|ジュース|ぶどう|コロロ|パインアップル|アップルサラダ',"苹果 1kg",'Mサイズ 1 玉按产地 10 kg 箱 40 玉 = 250 g；大玉按 32 玉 = 313 g'),
 ...
 'fish1kg':   ('鮭','円/kg','slice',r'小切れ|純米酒|料理専用|淡麗|スモーク',"鱼 1kg（塩鮭/銀鮭切身）",'1 切按 80 g 折（イオン同类商品标注「切身1切・80g」，ライフ不标）'),
...
def unit_price(key,r):
    ...
    if how=='piece':
        ...
        if key=='apple1kg':
            if 'Mサイズ1玉' in name: return p/0.25,'M 1 玉 ≈250 g（40 玉/10 kg）'
            if '2玉入' in name: return p/0.626,'大玉 2 玉 ≈626 g（32 玉/10 kg）'
        return None,''
    if how=='slice':
    ...
    return None,''
STAT={'flour1kg':('小麦粉 1袋·1kg',334),'oil1l':('食用油 1本·900g',412),'potato1kg':('じゃがいも 1kg',651),'tomato1kg':('トマト 1kg',771),'cabbage1kg':('はくさい 1kg',193),
