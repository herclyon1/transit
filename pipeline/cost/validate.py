...
# -*- coding: utf-8 -*-
"""
城市数据自检：岗位时薪必须带帖子写明的工时（basis=posted）；「上一休一」没写班长的不折时薪（2026-09-15 用户定，rule_24h/rule_12h 废除）；
...
for f in sorted(glob.glob('cost/data/cities/*.json')):
    ...
    for j in d.get('jobs',[]):
        ...
        if not h or h.get('basis')!='posted':
            print(f'{f}: 岗位「{j["chain"][:20]}」时薪 {w.get("value")} 没有帖子写明的工时（hours.basis 必须是 posted；rule_24h/rule_12h 2026-09-15 起废除）'); bad+=1; continue
        pa=w.get('posted_at')
        if not pa:
