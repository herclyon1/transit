...
# -*- coding: utf-8 -*-
"""
城市数据自检：岗位时薪必须带帖子写明的工时（basis=posted；「上一休一」按 PLAN-v2 口径折 24 h / 12 h 的记 rule_24h / rule_12h，帖子至少写了班制和班数）；
...
for f in sorted(glob.glob('cost/data/cities/*.json')):
    ...
    for j in d.get('jobs',[]):
        ...
        if not h or h.get('basis') not in ('posted','rule_24h','rule_12h'):
            print(f'{f}: 岗位「{j["chain"][:20]}」时薪 {w.get("value")} 没有帖子写明的工时（hours.basis 必须是 posted / rule_24h / rule_12h，不允许 assumed）'); bad+=1; continue
        if h.get('basis')!='posted' and not (h.get('days_per_month') and h.get('posted')):
            print(f'{f}: 岗位「{j["chain"][:20]}」basis={h.get("basis")} 但帖子没写班数/班制'); bad+=1; continue
        pa=w.get('posted_at')
        if not pa:
