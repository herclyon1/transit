#!/bin/zsh
# Acceptance views scored with the acceptance session's metric (styl-work/cmp-accept.py, threshold 40) and,
# for the dark views, the same recipe at threshold 20 (pipeline/basemap/cmpdiff.py --t 20): dark colours differ
# by small absolute amounts, so threshold 40 alone hides them (acceptance, 2026-09-16 evening).
...
T20=(python3 pipeline/basemap/cmpdiff.py)
...
echo -n "japan  dark  t20                   "; "${T20[@]}" "$OUT/japan-dark.png" ~/Money/styl-work/snap-japan-dark.png --t 20
...
echo -n "osaka  dark  t20                   "; "${T20[@]}" "$OUT/osaka-dark.png" ~/Money/styl-work/snap-osaka12-dark.png --t 20
