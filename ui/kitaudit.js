...
(function(){
  ...
  rule({name:'工具条玻璃钮 36 胶囊（右列 @右 8）；返回目录 = 侧栏底 Push button Regular 24 无边框', ref:'macOS 27 Kit › Titlebars and Toolbars › XL 36；Buttons › Regular 24', plat:'mac', sel:'.bar .btn-glass, .map-ctl .btn-glass', check:el=>{ const r=rect(el); if(el.matches('.bar > a:first-child')) return near(r.height,24)&&near(R(el),6)&&near(r.left,16)&&near(innerHeight-r.bottom,8)?[]:[`${num(r.width)}×${num(r.height)} r${num(R(el))} @(${num(r.left)},底 ${num(innerHeight-r.bottom)})≠24 r6 @(16,底 8)`]; return near(r.height,36)&&isCapsule(el,r)&&near(innerWidth-r.right,8)?[]:[`${num(r.width)}×${num(r.height)} 右 ${num(innerWidth-r.right)}≠36 胶囊 @右 8`]; }});
