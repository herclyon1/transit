...
(function(){
  ...
  rule({name:'侧栏/Popover 里的关闭 = 工具条玻璃钮 Medium 24（红绿灯只属于窗口和 Utility Panel）', ref:'macOS 27 Kit › Titlebars and Toolbars › Buttons › Medium 24', plat:'mac', sel:'.sheet:not(.split) .head .btn-glass, .sheet.opt .head .btn-glass', check:el=>{ const r=rect(el); return near(r.width,24)&&near(r.height,24)?[]:[`${num(r.width)}×${num(r.height)}≠24`]; }});
  rule({name:'面板红绿灯（Stoplights 44×10：三个 10 圆 @0/17/34，关不了的灰 15%）', ref:'macOS 27 Kit › Titlebars and Toolbars › Utility Panel › Stoplights', plat:'mac', sel:'.sheet.split #card .head .btn-glass',
