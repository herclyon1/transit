...
(function(){
  ...
  const VV=window.visualViewport; let W,H,OY,wide,safe,root,INSET;
  function run(){
    W=window.innerWidth; H=VV?VV.height:window.innerHeight; OY=0; /* 全站已不用 position:fixed，rect 就是屏幕坐标；visualViewport.offsetTop 在网页 App 里是 -安全区，只影响 fixed 元素 */ wide=W>=900; safe=safeNow(); root=px(cs(document.documentElement).fontSize); INSET=W>=414?20:16;
    R.length=0;
    ...
    const bar=q('.bar'); if(bar){ const b=qa('.bar .btn-glass')[0]; if(b){ const r=rect(b); ok('工具条按钮高 44', r.height, 44, 0.5); ok('工具条按钮宽 ≥44', r.width>=43.5, true, 0, num(r.width)); if(wide){ const rb=Math.max(...qa('.bar > *').filter(b=>!b.classList.contains('spacer')).map(b=>rect(b).right)); ok('工具条靠右 24（iPad 地图 App 控件簇）', W-rb, 24, 0.5); ok('工具条顶 = 安全区 + 8', Math.min(...qa('.bar .btn-glass').map(b=>rect(b).top)), safe+8, 0.5); } else { ok('工具条左内缩 = 布局边距 '+INSET, r.left, INSET, 0.5); ok('工具条按钮顶 = 安全区', r.top+OY, safe, 0.5, 'safe-top '+num(safe)); } } }
    // Sheet
    const sh=q('.sheet'); if(sh){ const r=rect(sh); const d=sh.dataset.detent||'medium';
      ...
      const s=qa('.sheet .search')[0]; if(s){ const sr=rect(s); ok('搜索胶囊高 44', sr.height, 44, 0.5); ok('搜索胶囊距 Sheet 内边 16', sr.left-r.left, 16, 0.5); ok(wide?'搜索胶囊距面板顶 16':'搜索胶囊距 Sheet 顶 15', sr.top-r.top, wide?16:15, 0.5); ok('搜索胶囊左内边 11', px(cs(s).paddingLeft), 11, 0.1); }
      const head=qa('.sheet .head')[0]; if(head){ const hb=qa('.sheet .head .btn-glass')[0]; if(hb){ const br=rect(hb); ok('卡片头圆钮 44', br.width===44&&br.height===44, true, 0, num(br.width)+'×'+num(br.height)); ok(wide?'卡片头圆钮距面板顶 16':'卡片头圆钮距 Sheet 顶 15', br.top-r.top, wide?16:15, 0.5); ok('卡片头圆钮距 Sheet 边 15', Math.min(br.left-r.left, r.right-br.right), 15, 0.5); }
    ...
    const ch=qa('.chips button')[0]; if(ch){ ok('筛选胶囊 32', rect(ch).height, 32, 0.5); ok('筛选胶囊间距 8', px(cs(ch.parentElement).columnGap||cs(ch.parentElement).gap), 8, 0.1); const on=qa('.chips button[aria-pressed="true"]')[0]; if(on) ok('选中胶囊 tint 白字', cs(on).color==='rgb(255, 255, 255)', true, 0, cs(on).backgroundColor); }
