...
(function(){
  ...
  function run(){
    ...
    R.length=0;
    ok('根字号 = 17（-apple-system-body；桌面也不缩，用户 09-15）', root, 17, 0.01, 'Chromium 会是 16');
    ...
    const sh=q('.sheet'); if(sh){ const r=rect(sh); const d=sh.dataset.detent||'medium'; const vis=Math.min(r.height, H-r.top);   /* ui/sheet.js 用 transform 定位，元素本身永远是大档高 */
      ...
      } else if(MAC){ const panel=sh.classList.contains('panel-wide'); ok(panel?'面板宽 380（桌面，用户 09-15）':'侧栏宽 360（桌面，Maps Mac 侧栏 320–380）', r.width, panel?380:360, 0.5); ok('侧栏左 8', r.left, 8, 0.5); ok('侧栏顶 8', r.top, safe+8, 0.5); ok('侧栏底 8', H-r.bottom, 8, 0.5); ok('侧栏圆角 14', px(cs(sh).borderTopLeftRadius), 14, 0.5); const cp=q('.sheet.split #card'); if(cp&&!cp.hidden){ const cr=rect(cp); ok('地点卡片面板 380 宽、贴侧栏右 8', Math.abs(cr.width-380)<0.5&&Math.abs(cr.left-r.right-8)<0.5, true, 0, num(cr.width)+' left+'+num(cr.left-r.right)); const sn=qa('#card .stat .n'); sn.forEach((n,i)=>ok('统计卡数字不截断 #'+i, n.scrollWidth<=n.clientWidth+1, true, 0, n.textContent.trim())); }
    ...
    if(MAC){ // 桌面（2026-09-15 用户裁定：字号/行高/卡片同手机，只有侧栏 360、面板 380、工具条 36、开关 38×22 不同）
    ...
    {   // 地图 App 的 Sheet 内容组件（DESIGN-HIG.md「地图 App 组件」）——手机和桌面同一套（用户 09-15）
