...
## 2026-09-17 23:30　用户死命令：复制逻辑不复制瓦片——验收失职记录　验收人：transit 验收（Fable）

我放行的 63308c3（地类/气候/高程栅格）、d0c6c2c（每要素 min-zoom）、16cbf29（东亚类栅格）都是把苹果瓦片内容搬进网站，地图无限大抄不完；错在验收没分清「规则」与「内容」。纠正：CLAUDE.md 加死命令；数据会话最优先用 MODIS/Köppen/terrarium 按解出的规则重出同名栅格并删苹果派生文件；界面会话先修球缘光晕，等新栅格替换。保留的解出物：样式表、着色公式、DvMt 色表、气候格、标注排布规则（待解）。
