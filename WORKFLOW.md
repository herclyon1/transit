...
### 验收必做三步（2026-09-16 22:1x 用户指出弹窗透明度这种一眼可见的差异不该由他发现，加的硬规矩）
1. **先看再量**：并排图（App 同视野 / 我们）自己看一遍，按元素逐个写「一眼差在哪」（材质透不透、字粗细大小、颜色深浅、位置），写不出差异才算看过。DOM 探针只用来把看到的差异量成数字，不能替代看。
2. **材质与颜色同坐标采样**：在黑太空区量有效白 α，在海面/陆面区量色偏与模糊提亮；数字与 computed style 一起写进记录。
3. **对照物必须是 App 同状态截图**，Kit 数字只当起点；Kit 与 App 实测不一致时以 App 为准并注明。
每条放行记录必须含以上三项，缺一项不许放行。

- **验收会话发信后必核送达**：返回 `queued` 视为未送达，立刻查对方记录；有时效的信（打回、方法变更、停手）没到就中断对方回合让它进；只有在对方记录里看到才算「已通知」。（09-16 两次信压了半小时以上没人发现）
...
- **材质（透明度/模糊/饱和）验收法**：两图同视野同坐标，取压在黑太空上的区域比像素（黑底上有效白 α = 像素/255），再取海面区看模糊提亮；不在亮底或白盒上量（09-16 弹窗「70%」就是这么量错的）。
...
## 七、不自设时间限制（2026-09-16 用户定）

不给任何一条线设「封顶几天」，也不在报告里写「用了几天/还剩几天」；只报做了什么、指标、下一步。时间由用户记录。

## 八、复制逻辑不复制内容；拟合与验收分开（2026-09-16 深夜用户定，验收会话失职后补）

- 从苹果文件解的只能是**规则**：样式表数值、着色公式、材质色表、气候格、标注排布逻辑。苹果瓦片里的内容（地类/气候/高程栅格、要素与 POI 记录、每要素 min-zoom、名单）是**数据**，不进网站、不进 map/data、不进 geojson 字段；只放仓库外做校准。
- 网站数据只来自第三方，用解出的规则映射成苹果的类和级别。凡是按某个视野拟合出来的规则（城市档位、街区折叠、海底要素类别）一律标「拟合替代」，改由数据本身（人口、SCALERANK、面积）推出，等排布逻辑解出后换成它。
- **拟合视野与验收视野必须分开**：大阪拟合就用名古屋、福冈、首尔验；东亚球拟合就用欧洲、北美、南美验；至少三个未参与拟合的视野。
- 派单不许给截图名单当目标，只许给「找出决定哪些要素几级出现的规则，写出处」。
- 数据类验收顺序：① 规则是什么、从哪解的；② 拟合之外的视野命中率多少；③ 最后才看图。ACCEPT-LOG 记规则与命中率，不记「这几个名在框里」。放行前问：这个文件里有没有苹果瓦片的字节？有就退回。

## 九、验收标准与验收会话的边界（2026-09-17 凌晨用户定，经对接会话转达）

1. **标准从「像素一模一样」改为「样式规则一致」**：解出的规则都用上；苹果私有数据层——POI、盾牌、车站图标、地块多边形、标注排布——写进 PLAN-ONE-MAP §5 为「不做」，不再追。验收按规则表逐项验（哪条样式规则出哪个色、哪级显示什么），不按像素百分比，不数每视野标注。
2. **验收只验不派**：验收清单只来自用户过的 PLAN-ONE-MAP 和一致清单；任何方向变化（并行开 2D、换数据源、换做法）验收会话不许决定，写「待用户定」发对接会话。
3. **参照图登记制**：任何当参照的 App 截图先在 ACCEPT-LOG 登一条——文件名、App 模式（右列工具栏图标裁片留证）、侧栏/3D 状态、窗口尺寸、m/px 或比例尺读数、外观亮暗、URL、谁核过；两个干活会话各回「核过」才生效。
4. **每次收工** `pmset -g assertions | grep -E "PreventUserIdleDisplaySleep|NoDisplaySleep|UserIsActive"` 查一遍写进 ACCEPT-LOG（§三已定，保留）。
5. **额度**：每个功能单元开工前，干活会话先发对接会话一行「做什么、为什么、怎么验」，看过才开；不并行开 2D；做法变更必须记录并报。
6. **对接**：用户不直接看验收会话；验收会话的结果、状态、问题一律发对接会话（现为「transit · 数据与底图」），对话里只留一行「已发」；验收会话也不写中间回复。
