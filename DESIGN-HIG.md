...
| 薪资·购买力 `cost/` | 地图 App（HIG Maps › Place cards：iOS 用 sheet，iPadOS/macOS 用侧栏） | 地图全屏；顶部左 44 玻璃圆钮（返回）、右玻璃 pop-up（货币）；底部非模态 Sheet：列表态 = 搜索胶囊 + 城市分组列表，卡片态 = 城市标题 + 关闭钮 + 分组列表（render() 的 section/h3 映射成 sect/h2/group）；≥900px 变左侧 320 侧栏 |
| 大阪·居住 `osaka/` | 地图 App + 设置 App 的表单行 | 地图全屏；返回钮；Sheet 里是 阈值滑块行、可达枢纽数分段控件、四个开关、居住等级图例、可达枢纽图例；主题跟随系统（去掉了手动明暗按钮：HIG Settings「不要做冗余的系统设置」） |
...
| 宽屏 | ≥900px：Sheet 变左侧 320 宽、距上 64（工具条之下）距左 16 的侧栏，圆角 26，第一行距顶 15 | HIG Sidebars / Maps（iPadOS/macOS）；320 未量 |
...
| HIG 页 | 规则（浓缩） | 页面怎么做的 |
|---|---|---|
| Maps | 别用不可交互的东西盖住地图；自定义控件要靠描边/阴影和地图区分；搜索配筛选；选中要有明显样式；place card 在 iOS 是 sheet、iPadOS/macOS 是 popover/侧栏 | 地图全屏，所有控件是玻璃（有阴影和内描边）；城市卡片是非模态 Sheet，宽屏侧栏；选中的城市点靠 MapLibre 样式区分 |
...
4. **宽屏**：桌面预览面板 1200×800 打开 `<页>?accept` 看侧栏项（宽 320、左 16、顶 = 安全区 + 64、圆角 26）；这一关只看几何，不看字号。
...
- **未做**：导航栈的滑动返回（这些页没有栈）；tab bar（每页是单一视图）；macOS Maps 的侧栏数字没量（本 Mac 的 Maps 空账号打不开侧栏），
