...
| our rebuild | WebGL post-pass `map/globe-light.js` (2026-09-17): the corona is drawn in screen space from the decoded constants — 150 km thick at the limb's scale, `colorMidPoint` 0.5 on the silhouette: inner half `mix(horizonColor, midColor, t1)` opaque over the disc's last 75 km, outer half `mix(midColor, black, t2)` over 75 km outside, × `lightColor·0.25·(L·pos+1)² + ambient` (far branch); colours the sheet's `Sky-Standard-Day` prop 202 / fillColor (Night likewise), linearised — byte-identical to the captured `skyBottomColor` / `skyTopColor`. Verified on `native-nosidebar.png` at 2x (`map/README.md` "Rim geometry"): the App's ocean texture stops 12.5 px inside the silhouette and the corona runs opaque from there (R 154 → 122 at the silhouette → 0 at +16 px); k = −8 App (146,162,184) vs formula (146,165,185), k = 0 (122,146,175) vs (120,152,183). The earlier fitted haze table is gone. |
| gap / fix | the outside glow is fully specified above: at the acceptance camera (D = 2.89 earth radii ⇒ h = 1.89 R ≫ 150 km) the far branch sets the corona's outer edge to R + 150 km = 1.0236 R, i.e. **2.4 % of the silhouette radius ≈ 14 px at the 578 px (1×) silhouette = 150 km**; what the UI measured on `native-nosidebar.png` is 14 px **@2× = 7 pt = 75 km — the outer half of the corona (mid colour → black)**: the inner half (horizon colour → mid) sits over the disc's own edge pixels and reads as part of the limb. Colours rgb(212,226,240) at the surface → rgb(155,196,237) at half the thickness (colorMidPoint 0.5) → black at the edge, multiplied by the sun term (brighter on the lit side, `lightingEnabled` = 1 in the far branch). Done (post-pass). Measured: the inner half **is** over the disc (opaque, ocean texture ends there), but its visible width is 12.5 px @2x (≈ 69 km) starting at t1 ≈ 0.2, the outer half 16 px (≈ 88 km): the ring's plane / radii (`AtmosphereMesh`, `buildAtmosphereModel`) are not decoded — a flat ring in front of the horizon plane, depth-tested against the globe, would clip the inner edge and stretch the outer half like this. **Open for the data session: the mesh vertex placement.** The page uses the symmetric ±75 km. The *inside* darkening is the lighting term (§2.5) plus the ground atmosphere term (§3.1 of SHADER-NUMBERS: hg = (2.0, 0.5) captured, `fogParameters.w` for globe tiles not captured → **w = 1.15 fitted**, rms 0.0446 linear on 108 deep-ocean samples; `map/globe.js ATMOS_W`). |
...
| look | per tile VectorKit builds the palette texture (`styleTexture`, 3 × 3·classes RGBA8) from the tile's material list: each material's **`DvMt` colour table** (prop 8 rgba by zoom band, conditioned on day/night and map style — `ui/basemap/dvmt-materials.json`; the style-2 bands are byte-for-byte the sheet's **`Landcover-<Class>.Light-Elevated` / `.Dark-Elevated` `fillColor`**, so either source gives the flat map's numbers; the globe uses style 0) [dvmt, styl `default-iosmac-11358.styl`], linearised: Forest z5 rgb(176,222,144) → `#6dba47` lin, Wetlands rgb(175,230,168) → `#6bc963`, Cultivated rgb(199,235,152) → `#90d34e`, Herbaceous rgb(188,230,151) → `#7ec94d`, Shrubland rgb(228,235,192) → `#c1d37b`, Barren rgb(240,236,216) → `#dcd3ad`, IceSnow rgb(245,245,245) → `#e8e8e8`, Ground rgb(235,237,223) → `#d3d5bc`, Water → alpha 0 (§2.3). The sheet colours step by zoom band (`Landcover-Forest.Light-Elevated`: z1 rgb(146,213,98), z3 rgb(163,217,125), z5 rgb(176,222,144), z8 rgb(180,224,149) …) — the globe is z ≈ 2–4, so the z1–z4 rows apply. |
...
| globe ocean + land below z 4.6 — the Mac App's pastel (`palette-globe.json`) | the DaVinci **material tables for map style 0** | **decoded** (`pipeline/basemap/dvmt.py`, `ui/basemap/dvmt-materials.json`): every ground material's `DvMt` resource carries colour bands by zoom under condition sets (client:1 TimePeriod day/night × client:69 map style 0 / 1 / 2 [× 135 = 1, 0 = 3]); **style 2 = the flat sheet's `Landcover-*` colours** (Forest z0–3 #92d461, z4–7 #afde90; Ground #ebecdf; Water ramp 0 m #91daf3 … = the captured `gradient1Texture`), **style 0 = the globe**: Ground #f6f6ed, Forest #deecd3 (z4–10), Herbaceous #e1eed6, Barren #f6f3e3, Water ramp 0 m #bce8f8 → 7 km #5bb9ee. × light(0,0,1) against the sampled Mac globe: Forest (226,241,215) vs humid (233,246,216), Herbaceous (229,243,218) vs semi-humid (235,242,217), Barren (251,248,232) vs very-dry (248,244,227), Ground (251,251,242) vs high-grey (246,248,237); ocean 0–200 m (179,230,252) vs (191,228,247), 1–2 km (157,217,248) vs (169,214,241), 5–6 km (96,194,244) vs (119,188,233) — land within 7, sea within 12 shallow / 23 red deep (the remaining term is the globe lighting / haze, not the palette). iOS draws the style-1 tables. `spr_globe.py --dvmt-style 0 --zoom 4` paints the globe rasters with them (output kept outside the repo). `palette-globe.json` can be retired once the UI wires the numbers |
...
| gap | replace by 240°/65° and `groundElevationScale(z)` applied to the DEM (or its equivalent in MapLibre's exaggeration once the amplitude mapping is written down once); `normalsSharpnessBias` — its use in the mesh normal computation is not decoded (VectorKit `DaVinciGround*` normal builder), so the *softness* of the relief at z12 is still by eye. **Unit 3 (2026-09-16 night):** `pipeline/basemap/shade.py` writes the light factor `light(n)/light(0,0,1)` per pixel for the East-Asia box (`map/data/shade-ea-z{5,6,7}.png`, 8-bit v/128, AWS terrarium z7 DEM, s = 3.25 / 2.5 / 1.5, the decoded formula and numbers, `ui/basemap/shade.json`) for the UI to multiply the linear albedo with instead of MapLibre's hill-shade. Check against the App's Japan view (forest pixels of the Japanese Alps, `snap-japan.png`): App linear factor p5 / p50 / p95 = 0.73 / 0.95 / 1.08 of the flat Forest colour; the formula on the 1 km DEM with s 2.5 gives 0.83 / 1.00 / 1.05 (3 km smoothing 0.90 / 1.00 / 1.04) — the App's slopes are shaded about twice as deep. What the formula lacks: the **ground shadow map** (`DaVinci::groundShadowMap_*` in the metallib: `light = occlusion·indirect + shadow·direct`, SHADER-NUMBERS 3.2 — cast shadows of the terrain, not captured) and `normalsSharpnessBias`; a tilt exaggeration of 2 on the 1 km slopes reproduces the App's spread (0.67 / 0.98 / 1.05) and is the stand-in until those are decoded. The z6 SPR meshes (~3 km vertices) cannot carry that relief themselves: the formula on Apple's own z6 mesh gives p5 0.94. |
...
| our rebuild | N02 centre lines from `tiles/transit.pmtiles` (`rail`, `cls`); v6: casing dash per zoom from the 280 rows (`[4,8]`…`[4,32]` × 0.2 pt ÷ casing width); v6.1: the 279 dash (whole line) also dashes the casing; v6.2: butt caps on rail core and casing (round caps on a dashed casing put a blue dot past every core dash) — together these give the App's pale, thin shinkansen at z6–8; the sheet's white core 1 + blue stroke 0.5 (no ×1.2987) was right all along [`to_maplibre.py`]. |
...
| line names (English) | The `rail` layer now carries `name_en` + `op_en` (`pipeline/japan/rail_name_en.py`, review sheet `pipeline/japan/rail-name-en.tsv`): 556 of the 602 N02 (operator, line) pairs matched to OpenStreetMap railway ways' `name:en` (121 000 ways, ODbL; route relations as a second source) after normalising both sides (OSM's operator prefix and 本 — 「JR山陽本線」 = N02 「山陽線」; subway 「1号線(御堂筋線)」 → 御堂筋線; operator must agree when tagged). English split into `op_en` = brand (JR, Hankyu, Osaka Metro, Kobe Electric Railway) and `name_en` = the line (Kyoto Line, Midosuji Line, Tokaido Main Line); label = `op_en + " " + name_en`, or `name_en` alone when empty — shinkansen have no `op_en` because the App writes "Sanyo Shinkansen" (acceptance's screen, no JR). Macrons removed (Sanyō → Sanyo); 15 OSM spellings corrected (listed in the script). The 46 unmatched are trams (Sapporo/Kumamoto/Kagoshima/Toyama city lines), 7 cable cars and small third-sector lines OSM has no `name:en` for. **The wording is OSM's, not Apple's** — the App's transit label strings (`Japan-Railway-Label-Base` / `Bullet-Label-Base` text) are in the SPR_ROADS label chapter and not decoded yet. The tiles were rewritten in place with a byte-level MVT rewriter (`pipeline/japan/mvt_retag.py`, no tippecanoe on this machine): every other byte of every tile is unchanged (checked: 52 147 tiles, 0 geometry / property differences, 271 028 rail features tagged). |
...
| look [styl] | **cities** `City-Label-LMZ-05` (largest): height 10.5→12.5 (z5–7), 14→16 (z8–9), 16→28 (z10+), font medium-G1 (z5) → semibold (z6–7) → bold (z8–11) → semibold (z12+), width=90, colour `#1c1c1c` (≤ z11) → `#5a5e5e` (z12+), halo `#f8f8f6` α0.8; `LMZ-07`: 10.5 (z5), 9.5→10.5 (z6–7), 10.5→12.5 (z8–9), 15→27 (z10+), medium-G1 → semibold (z10–13), colour `#4c4c4c` → `#1c1c1c` (z8) → `#5a5e5e` (z12); `LMZ-09`: 12.5→14.5 (z10), 17→23 (z12), medium-G3 at z12; `LMZ-12`: 13→15 (z12), 15→17.5 (z13), 17.5 (z14). **Prefectures** `State-Label-Small`: visible z7–10 only, 13→16 pt, semibold width=90, `#7c365f`, halo `#f8f8f6` α0.8. **Wards** `SubMuni-Ward`: visible z10–14, 10.5→12.5 (z10), 14→16 (z12), 16→18 (z13), `%$default,semibold,width=60` (a 60 %-width face), `#5a5e5e`, no halo, `textSizeScale` 0.45–0.5. **Countries** `Country-Label-Medium`: z5 14→15, z6 15→16, z7+ 16 pt, `%$default,bold-G3,width=80`, `#7c365f`, halo `#f8f8f6` α0.8 (the App's "SOUTH KOREA" caps measure 10 px = 14–15 pt Latin capitals; our 日本 at the same 15 pt fills the em and looks larger — same number, different script). **Road names** §7.5. **Water names** `Lake-Label.Zoom9-*`, `Ocean-Points.Large-*` (`basemap/data/styl/README.md`). `labelColorLumAdjustment` (463/464) is applied after the colour. |
...
| gap | z5–8: OpenFreeMap has `place` city ranks but Apple's LMZ classes are its own ranking — the rank→LMZ mapping is inferred; `width=60/90` faces and `medium-G1/G3` weights do not exist in Noto → nearest weight; `suburb` (ward) only from z12 tiles; which labels win is Apple's collision solver. **Physical / undersea names (unit 3):** the App's trench / trough / basin / ridge names at z5–7 are spread text along the feature (`PhysicalFeature-Rank-3-5` for rank-3 features etc.: `%$default,bold-G3,width=140`, rgb(124,112,106), halo rgb(248,248,246) α0.8, heights 14.3 (z4–5) → 16.9 (z5–6) → 19.5 (z6–7) → 22.1 Mac pt, `basemap/data/styl/flat-physical-label-styles.tsv`); deeps `PhysicalFeature-Undersea-Points-Base` (semibold width=100, 12.3–13 pt). Data: `map/data/undersea.geojson` now carries `apple_minzoom` (which the App names from which zoom, 38 matched) and, for polygon features, `axis` = the polygon's long axis for the baseline (lines keep their LineString); `physical.geojson` likewise (`apple_minzoom`, 41). **Neighbourhood names (z11–13):** the App's small uppercase names at Osaka z12 (NAGASU, KANDA, JUSO, HONJO) are `Neighborhood-Base-Small` (visible from Apple z12 = MapLibre z11; `%$default,medium-G4,width=60` z12–14 then semibold, `textSizeScale` 0.45 → 0.5, heights 13.6 → 15.6 → 18.2 → 20.8 → 23.4 Mac pt per zoom, colour `Neighborhood-Light-Base` rgb(90,94,94) → rgb(99,103,103) (z14) → rgb(117,122,122) (z15), dark rgb(188,201,217)/rgb(200,214,231)/rgb(215,229,247) halo rgb(45,61,81); `Neighborhood-Base-Large` one zoom earlier). OpenFreeMap has no such layer below z14, so `map/data/neighbourhoods.geojson` is built from OSM `place=suburb|quarter|neighbourhood` in the Keihanshin box (`pipeline/basemap/neighbourhoods.py`, Overpass, ODbL). Japanese OSM tags every 町丁目 as `neighbourhood` (21 600 nodes, 11 600 「N丁目」), so the script folds them: **block** = one 町 (chome nodes with the same base name within 2 km; 13 094), **district** = blocks sharing a stem + directional/町 suffix (東/西/南/北/中/本町/元町/東通…; 583, e.g. 長洲東通+本通+中通+西通 → 長洲 NAGASU, 十三東+十三本町 → 十三 JUSO), **ward** = `suburb` (52). English = OSM `name:en` with chome numbers / the romanised suffix stripped and macrons removed (the App writes Osaka/Kyoto/Juso without them); `min_zoom` district 11, block ≥ 3 chome 12, else 13; `rank` = member count for collision priority. The folding rule is ours — the App's own neighbourhood polygons are not in the tiles we decode; it is checked only against the four App names above (all four come out as districts at z11). |
